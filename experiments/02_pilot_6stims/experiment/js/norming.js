...
function make_slides(f) {
  ...
  slides.clickObj = slide({
    ...
    present_handle: function(stim) {
      ...
      document.getElementById("img_3").style.border = "4px solid lightgrey";
      document.getElementById("img_4").style.border = "4px solid lightgrey";
      document.getElementById("img_5").style.border = "4px solid lightgrey";
      ...
      var dist3_html = '<img src="pictures/'+stim.dist3_type+'_'+stim.dist3_color+'.png" style="height:110px;">';
      var dist4_html = '<img src="pictures/'+stim.dist4_type+'_'+stim.dist4_color+'.png" style="height:110px;">';
      var dist5_html = '<img src="pictures/'+stim.dist5_type+'_'+stim.dist5_color+'.png" style="height:110px;">';
      shuffled_images = _.shuffle([target_html, dist1_html, dist2_html, dist3_html, dist4_html, dist5_html]);
      ...
      var dist3_pos = shuffled_images.indexOf(dist3_html);
      this.stim.dist3_pos = dist3_pos;
      var dist4_pos = shuffled_images.indexOf(dist4_html);
      this.stim.dist4_pos = dist4_pos;
      var dist5_pos = shuffled_images.indexOf(dist5_html);
      this.stim.dist5_pos = dist5_pos;

      // jquery pictures to html
      $("#img_0").html(shuffled_images[0]);
      $("#img_1").html(shuffled_images[1]);
      $("#img_2").html(shuffled_images[2]);
      $("#img_3").html(shuffled_images[3]);
      $("#img_4").html(shuffled_images[4]);
      $("#img_5").html(shuffled_images[5]);
    ...
    log_responses : function() {
        exp.data_trials.push({
          ...
          "dist3_type" : this.stim.dist3_type,
          "dist3_color" : this.stim.dist3_color,
          "dist4_type" : this.stim.dist4_type,
          "dist4_color" : this.stim.dist4_color,
          "dist5_type" : this.stim.dist5_type,
          "dist5_color" : this.stim.dist5_color,
          "target_pos" : this.stim.target_pos,
          "dist1_pos" : this.stim.dist1_pos,
          "dist2_pos" : this.stim.dist2_pos,
          "dist3_pos" : this.stim.dist3_pos,
          "dist4_pos" : this.stim.dist4_pos,
          "dist5_pos" : this.stim.dist5_pos,
...
function init() {
  ...
  function makeStim(item_entry, context_condition) {
    ...
    // each context also contains 3 random distractor, which (again) can't share the target's
    // color or type and can't be the exact same object compared to other distractors
    // initialize with target values to start while loop
    dist3_type = new_target.target_type;
    dist3_color = new_target.target_color;
    while (dist3_type == new_target.target_type || dist3_color == new_target.target_color ||
          (dist3_type == dist1_type && dist3_color == dist1_color) ||
          (dist3_type == dist2_type && dist3_color == dist2_color)) {
      // is this the correct range?
      random_dist_entry = getRandomInt(0,21);
      dist3_type = items[random_dist_entry].target_type;
      dist3_color = items[random_dist_entry].target_color;
    }
    dist4_type = new_target.target_type;
    dist4_color = new_target.target_color;
    while (dist4_type == new_target.target_type || dist4_color == new_target.target_color ||
          (dist4_type == dist1_type && dist4_color == dist1_color) ||
          (dist4_type == dist2_type && dist4_color == dist2_color) ||
          (dist4_type == dist3_type && dist4_color == dist3_color)) {
      // is this the correct range?
      random_dist_entry = getRandomInt(0,21);
      dist4_type = items[random_dist_entry].target_type;
      dist4_color = items[random_dist_entry].target_color;
    }
    dist5_type = new_target.target_type;
    dist5_color = new_target.target_color;
    while (dist5_type == new_target.target_type || dist5_color == new_target.target_color ||
          (dist5_type == dist1_type && dist5_color == dist1_color) ||
          (dist5_type == dist2_type && dist5_color == dist2_color) ||
          (dist5_type == dist3_type && dist5_color == dist3_color) ||
          (dist5_type == dist4_type && dist5_color == dist4_color)) {
      // is this the correct range?
      random_dist_entry = getRandomInt(0,21);
      dist5_type = items[random_dist_entry].target_type;
      dist5_color = items[random_dist_entry].target_color;
    }
    ...
    return {
      ...
      "dist3_type" : dist3_type,
      "dist3_color" : dist3_color,
      "dist4_type" : dist4_type,
      "dist4_color" : dist4_color,
      "dist5_type" : dist5_type,
      "dist5_color" : dist5_color,
      "condition" : condition,
      "utterance" : utterance
    }
  } 
